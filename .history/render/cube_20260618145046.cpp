...
public:
    RotatingCube() {
        ...
        // Cubo com aresta de tamanho 2, centrado na origem
        initializeVertices();
    }
    ...
    Point2D project(const Point3D& p) {
        ...
        double x2d = p.x * scale * SCALE + SCREEN_WIDTH / 2.0;
    ...
    void update() {
        ...
        // Ordena por profundidade (depth sorting para renderização correta)
        sort(projectedVertices.begin(), projectedVertices.end(),
    ...
    void draw() {
        ...
        // Desenha as arestas (linhas entre vértices)
        drawEdges(screen);
        
        // Limpa a tela e imprime
        cout << "\033[2J\033[H"; // ANSI escape code para limpar tela
        ...
        cout << "Use Ctrl+C para sair" << endl;
    }
    
    void drawEdges(vector<vector<char>>& screen) {
        // Define as arestas do cubo (índices dos vértices)
        int edges[12][2] = {
            // Face frontal
            {0, 1}, {1, 2}, {2, 3}, {3, 0},
            // Face traseira
    ...
    void drawLine(vector<vector<char>>& screen, const Point2D& p1, const Point2D& p2) {
        // Algoritmo de Bresenham simplificado para desenhar linhas
...
int main() {
    ...
    while (true) {
        cube.update();
        cube.draw();
        std::this_thread::sleep_for(std::chrono::milliseconds(50)); // 50ms de delay (uns 20 FPS)
    }
